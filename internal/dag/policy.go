...
func outlierDetectionPolicy(globalOutlierDetection, serviceOutlierDetection *contour_v1.OutlierDetection) (*OutlierDetectionPolicy, error) {
	...
	if outlierDetection.Interval != nil {
		interval, err = time.ParseDuration(ptr.Deref(outlierDetection.Interval, "10s"))
		if err != nil {
	...
	if outlierDetection.BaseEjectionTime != nil {
		baseEjectionTime, err = time.ParseDuration(ptr.Deref(outlierDetection.BaseEjectionTime, "30s"))
		if err != nil {
	...
	if outlierDetection.MaxEjectionTime != nil {
		maxEjectionTime, err = time.ParseDuration(ptr.Deref(outlierDetection.MaxEjectionTime, "300s"))
		if err != nil {
	...
	if outlierDetection.MaxEjectionTimeJitter != nil {
		maxEjectionTimeJitter, err = time.ParseDuration(ptr.Deref(outlierDetection.MaxEjectionTimeJitter, "0s"))
		if err != nil {
